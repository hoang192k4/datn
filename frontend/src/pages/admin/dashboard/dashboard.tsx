...
const Dashboard = () => {
    const [statistics, setStatistics] = useState<StatisticsAdmin>({ total_teachers: 0, total_course_sections: 0, total_students: 0 });
    const user = useSelector((state: any) => state.auth.user);

    const fetchDasboard = async () => {
        try {
            const res = await getStatisticsForAdmin();
